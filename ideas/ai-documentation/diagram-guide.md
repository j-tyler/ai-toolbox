...
3. Write down meaningful absences: a transition that does not exist, a consumer that does not listen, a path that is not taken. Record the absences a reader would otherwise assume are present, not every possible gap.
...
- `->>`: a synchronous call. The caller waits for it.
- `-)`: an asynchronous send. The caller does not wait. Use this for every queue publish, event emit, and fire-and-forget call. This is the most important mark in the diagram, because it flags the exact step where a reader following the code loses the trail.
- `-->>`: a return value.
...
Retries and repeats come in two shapes, and they are drawn differently. A retry that happens in place, the same call attempted again by the same caller, is a `loop` block around the repeated message, with the bound in the loop label: `loop up to 3 attempts`. A retry that re-enters through a queue or a scheduler, a worker that re-enqueues its own job or a cron that picks the record up again next run, is drawn once as the asynchronous message back to the queue or scheduler participant, followed by a `Note over` stating the retry limit and the function that enforces it. Do not unroll repeats into copies of the same messages, and do not draw a second pass through the flow. If the limit is not enforced anywhere, say that in the note; an unbounded retry is exactly the kind of fact a reader needs. Around twelve messages is a comfortable length; if a scenario needs many more and has a natural seam, split it into two scenarios, and if it has no seam, keep it whole.
...
Every state a thing can be in, what moves it from one state to another, which states are final, and, by what is missing, which transitions are not allowed. The full picture usually exists nowhere in the code: the transitions are scattered across handlers, and the forbidden transitions are not written down anywhere.
...
- Are meaningful absences stated in comments, where a reader would otherwise assume something is present?
