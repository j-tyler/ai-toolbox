...
Do not try to prove that no imaginable implementation exists. Consider the plausible alternatives supported by the current repository and normal scope of this pull request. Before amending, name those alternatives in your implementation evidence. If any can satisfy the current approved baseline, change the implementation. An amendment may repair a claim or settle a missing decision only while preserving approved outcomes, constraints, and exclusions and honoring applicable project instructions. If affirmative evidence shows that every reasonable in-scope implementation would violate one of those boundaries or an applicable project instruction, stop and report a failed change; do not amend the boundary or instruction away. Uncertainty or unavailable production proof is not affirmative evidence of a conflict.
