...
- **Bounded-discretion exercise.** Granted latitude ("TTL may be anywhere in 10–60s, implementation's choice") is exercised silently; anything else that conflicts with the intent is amended if the intent is wrong, and not done otherwise. The agent never treats that gap as delegated discretion just because a default seems obvious.
