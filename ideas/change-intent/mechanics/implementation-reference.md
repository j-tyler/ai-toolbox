...
- **Halt-and-escalate, with the default inverted.** An agent's trained default under ambiguity is to pick a reasonable value and keep going. The instructions state plainly: when the change forces observable behavior the intent takes no position on, halting is the success condition, not an interruption. A structured escalation format — discovered fact, forced choice, options, recommendation — makes the halt a deliverable instead of a failure.
- **Bounded-discretion exercise.** Granted latitude ("TTL may be anywhere in 10–60s, implementation's choice") is exercised silently; everything else escalates. The agent never treats an escalation-worthy gap as delegated discretion just because a default seems obvious.
