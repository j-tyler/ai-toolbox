...
- **Session harvest.** The change was already discussed in this session. Harvest only what the author affirmed. Directions that were considered and discarded go under **Rejected in discussion**. If you cannot tell whether something was decided or merely discussed, it goes under **Deferred to exploration** — never presented as decided.
- **Cold start.** The request is the first answer: sort whatever outcomes, why, and constraints it already states into the template, and ask for what it does not supply inside the brief itself — one message, where the author's confirming reply settles the gaps and the gate together. Do not run a questionnaire. A bare one-sentence request is a cold start, not a harvest.
...
A required runtime behavior belongs in Acceptance criteria or Invariants according to its proof shape, not in Constraints merely because it is phrased as something to preserve. Implementation choices deliberately left open are omitted; they are not Out of scope. Author direction is for this change only unless the author says otherwise; when the author says a rule holds beyond this change, the durable statement they asked for is part of the change — recorded like any other requirement, in one of the durable homes the project's agents file lists.
...
2. **Decision candidates.** Keep a private scratch list through the pre-approval coverage pass. Begin with the consequence or shared author-owned property at issue, not the mechanisms that could provide it. Suppress a mechanism fork when its branches can uphold the same property. For each remaining candidate, record two reasonable consequence-level resolutions, whether it is change-defining, and whether approved direction or an explicit project constraint settles it. Only change-defining forks whose direction remains unresolved appear under Needs your attention. Rejected candidates never become author-facing bookkeeping, but do not discard them before the coverage pass — a later fact may change their classification. The private list may be discarded at approval. An empty list of change-defining forks is valid.
3. **Parked items.** Adjacent improvements you notice ("the error handling in the artwork upload path is also bad"). Never widen scope for them. They surface at approval as seeds for future intents.
...
- An applicable project instruction conflicts with the requested direction. The author's ruling must land durably: either the direction bends to honor the instruction, or the team changes the instruction file itself. Implementation and review treat instructions as binding as written, so an override that lives only in this dialogue will fail the change downstream.
...
- Deferred to exploration: <none, or each brief item that is neither in the
  draft nor under Needs your attention, with the short explanation that it
  was not change-defining>

Approve the proposal as a whole, approve it except for a named item, or
correct an item or draft line.
```

**Apply the change-defining test before triage.** Use it in private working notes:
...
A threshold, time window, or limit follows the same change-defining test as any fork. Ask the author only when choosing among reasonable values would decide which change is delivered. If every reasonable value is a way to deliver the approved change, leave it to implementation or state a bound supplied by approved direction or an explicit constraint; do not manufacture a range merely to make the intent look complete. A production-only operating bound can remain a constraint: record it precisely enough to guide engineering judgment, but do not ask the author to invent proof that implementation or review cannot obtain.

**Items under `Needs your attention` are written for an author who cannot read the code.** Before emitting each one, apply the test: *could the author rule from this entry alone, without opening a single file?* If not, rewrite it. Behavior consequences come before mechanisms; affected parties are named in product terms ("a song the listener just added wouldn't play"), with the code reference as evidence, not prerequisite.
...
Approval is explicit, and it attaches to text the author has seen. Approving the complete proposal approves its draft. When rulings have changed the draft since the author last saw it whole, re-emit the assembled file and take approval on that — the approved intent is never text the author has only seen as diffs. When taking approval, state what it means: implementation must honor the file's change-defining decisions, may choose ordinary implementation details, and may repair a false claim or missing necessary decision through recorded amendments, which the author rules on when the work comes back.
...
For an initial intent, write `change-intent/YYYY-MM-DD-short-slug.md` — today's date; a short, concrete slug at commit-title specificity, normally 3–6 words, using nouns about what changes rather than vague verbs about effort — and commit it before implementation begins, on the branch that will deliver this change. The intent travels with the work it governs — one branch, one pull request, one intent — so confirm you are standing where the change will be delivered before committing. How branches are created, named, and based remains the team's practice; when the destination is unclear, ask the author rather than guess. Use the shortest slug that clearly identifies the change; exceeding six words is not by itself a reason to stop or split. Offer to split the work when it requires independently deliverable intents. For a replacement, write the clean approved baseline at the existing path, remove the superseded candidate's Amendments section, and tell the implementing agent to reassess retained code and redo affected evidence before review runs again.
...
- A section with nothing to hold is omitted (Outcomes and Why always have something to hold). Absence is meaningful — an absent Amendments section means the current intent held as written during implementation.
- Amendments is absent from an initial or replacement baseline. It appears only if implementation amends that baseline. Each entry has a unique local identifier (`A1`, `A2`, ...), states the discovered fact, and quotes the complete affected item wording verbatim with its section under `Was` and `Now`. Use `Was: not present` for an item's first addition and `Now: removed` for a removal. A move names different sections. One discovery may contain several `Was`/`Now` pairs.
