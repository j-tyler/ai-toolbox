...
- **The amendment check as an algorithm.** The joins, spelled out, all inside the file: every Amendments entry names a discovered fact; every semantic entry has a discovery note on its claim and every discovery note has an entry; dates are consistent; relaxations also landed in Out of scope. The check reads the final file only — it never reconstructs the file's git history.
