...
AI generates code faster than humans can review it. A competent engineer with a good model produces thousands of lines a day; human review is sequential and attention-limited and scales roughly linearly. That gap is real today, and it's only going to widen. As models and agents get better, the rate of code generation accelerates while reviewer throughput stays roughly flat. A process that's already strained will be the complete bottleneck in six months, twelve months, two years. The point of this work is to improve review *today* in a way that still holds up under that trajectory — not a patch for the current moment but infrastructure built for where the field is going.

The real question isn't "when will AI be good enough to replace careful human review." It's "how do we develop a process that's actually verifiable, regardless of whether humans or AI run it." A process like that lets humans incrementally cede review to machines as confidence accumulates — the same arc autonomous vehicles followed. Waymo spent years with safety drivers in the seat while the system's decisions were measured against what a human would have done. It's now getting close to the point where it doesn't need any human assistance at all, in more and more cities. Code review is on a similar arc: at some point it will be done almost entirely by AI. The way we get comfortable with that future is by walking through it — and the walking-through looks like a human reviewer, seeing well-described changes that AI agents have already critically reviewed against the stated intent, gradually stopping to feel like they're adding value. When that feeling is consistent and trustworthy, the human role naturally recedes.

`CHANGE_INTENT.md` is one piece of that process. It's the macro-level artifact that captures what a change is supposed to do in a form that both humans and machines can verify against. Until the day human reviewers are no longer needed, it addresses several specific failure modes of today's review process:
