...
The author still rules on every amendment — at review rather than mid-implementation. When the work comes back, the author reads the Amendments section and accepts or rejects each repair. Rejecting one is a re-decision, not a revert — the discovered fact that forced the amendment still stands, so the original claim cannot simply be restored; the author supplies a different repair by reopening the intent ([Reopening the intent](#reopening-the-intent)), and the change runs the flow again. Moving the ruling to review has a known cost: a repair the author would have decided differently is discovered after the work is done, not before. The design accepts that cost deliberately — rework is cheap for an agent, and stopping mid-implementation to wait on a decision is expensive for everyone — and states it here so a future reader doesn't reintroduce mid-implementation approval as a fix.
...
Amendment covers the doing seat. A second reality needs absorbing: the deciding seat changes its mind. The author reads the returned work and refuses an amendment — the discovered fact still stands, so the original claim cannot simply be restored, and a different repair is needed. The reviewer looks at a finished implementation and sees a simpler shape the change should have had. The review pass faults the intent itself as underspecified in a way only its author can resolve. None of these pass the necessity test, and none need to: the necessity test binds the agent working inside the contract. It does not lock the author out of their own decision.

The design absorbs all of these with one move: **reopening**. Whenever the work is back on the author's desk, the deciding question may be reopened. The author revises the intent — through the same authoring dialogue that produced it, run only as far as the revision requires — approves the revision, and the change runs the flow again: implement, review, merge decision. The re-approval commit becomes the new baseline the review pass diffs against; amendment entries the author ruled into the revision are embodied in that baseline and stay on the record; the file keeps its name, date, and slug. One change, one pull request, one intent — now on its second lap.

Reopening is not an amendment. An amendment is an agent's repair to a contract that proved wrong, justified only by a discovered fact; a reopening is the author re-deciding, and an author needs no justification beyond having changed their mind. Nor is it a new change — the deliverable is still the same change, so it stays in the same file; a re-decision that abandons the change's outcomes for different ones is a new change, with its own intent and its own pull request. This coexists with the rule that improvements discovered mid-implementation become seeds for future intents: that rule governs the agent, mid-flight, with no authority to re-decide. The same idea on the author's desk is theirs to rule on either way — fold it into this change by reopening, or seed it as its own intent. The boundary that holds is the seat, not the idea. A reopening can even change nothing in the file: a reviewer redirecting the implementation without touching what the change must make true. If the preferred approach is a genuine requirement, it enters the acceptance criteria as a constraint, like any deliberate implementation choice; if it never rises to a requirement, the file doesn't move and the second lap is implementation only.

Reopening needs no rarity rule, and the contrast with amendments is worth stating precisely. Amendments must be rare because each is an agent's unreviewed judgment call, made under anchoring, inside the contract that exists to constrain it. A reopening is the opposite case: the judgment the whole pipeline exists to serve, exercised by the person it belongs to. Its cost — another lap of implementation and review — is its own regulator. And the anchoring that makes mid-implementation re-deciding suspect is, at the author's desk, often the point: some better shapes are visible only in a finished implementation, and a process that treats that moment as a violation would be brittle exactly where real teams live. Better ideas arrive on the second read; the design gives them a door, a record, and another lap — not a workaround.

In the worked example, the approved intent migrates AuditLog to a new `GetUserUncached` method. Suppose the reviewer, reading the finished change, sees a cleaner shape: invalidate the cache entry on every `UpdateUser` — no second method, and every caller gets immediate read-after-write, not just AuditLog. Nothing in the intent is wrong, so nothing here is an amendment. The author reopens: the `GetUserUncached` criterion comes out, an invalidation-on-write criterion goes in, the read-after-write invariant tightens from a 30-second window to immediate, the Why records why the second shape won, and the revision is approved and committed as the new baseline. Implementation runs again against the revised contract, review checks the new diff against it, and the change returns to the reviewer the same way it arrived the first time.
