...
**Every role has a defined path forward.** The process assigns an action to each unresolved condition: authoring may ask the author for a decision; implementation owns unconstrained technical choices and may amend the intent on the record when a claim proves wrong or a necessary change-defining decision is missing; review may report `cannot verify` or raise a finding without repairing the change. The process stops only when no repair can preserve the **approved boundaries** — the current intent's outcomes, constraints, and exclusions, together with applicable project instructions. The full table of situations and paths closes [the lifecycle chapter](#every-role-has-a-path-forward), where each entry can be read with the concepts it names already in hand.
...
Once its change merges, an intent file is frozen — never edited, never renamed, never deleted; it is a historical record of its own change, never a requirement on later ones. A decision that must outlive its change is carried forward by the change itself, in the durable homes the next paragraph names; a decision recorded only in a merged intent expires with it. Before merge, the current intent governs implementation and review; implementation may amend it and the author may replace it, under the rules in [the lifecycle chapter](#the-lifecycle-of-a-change). Follow-up changes to the same area get their own file with their own date and slug; the date prefix makes the lineage visible without making earlier intents govern later changes.

**Where durable rules live.** A rule meant to hold beyond its change is written into the system by the change that decides it, and its home follows from what the rule must do: a project instruction when the rule must bind the agents of every later change — standing policy goes here; a test when the rule is behavior the suite must mechanically enforce; a doc comment on the governing code when the rule must reach whoever next edits that surface. The author signals a standing rule in plain words — "always," "from now on," "our policy," or anything like them — and absent that signal, direction belongs to the change at hand.
...
- **Amendment count becomes a diagnostic.** Every amendment marks a spot where the author, the authoring dialogue, and the exploration of the affected code all missed something that implementation then exposed. One or two entries is normal — implementation always turns up surprises. Six entries on a small change points at the authoring dialogue (it missed real interactions with the affected code) or at the code itself (the area is genuinely hard to reason about), and either way the diagnosis is actionable: an area that accumulates amendments across changes has earned extra scrutiny in review, and a deeper authoring pass the next time a change touches it.
...
### Durable decisions are deposited, not searched

A merged intent never binds a later change, and no role searches the archive for decisions to preserve — at ten thousand changes as at ten. The design keeps that scaling property by placing the cost on the deciding change instead: a decision meant to outlive its change is written into the system as part of the work, in the durable homes [File location and naming](#file-location-and-naming) describes — where every later change already looks. The trade is stated plainly: if the deciding change does not make a decision durable, no later step recovers it. The frozen intent remains readable by anyone who wants the history, but nothing in the process requires the look.
