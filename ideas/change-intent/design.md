...
AI can now generate code at a rate that human review can't keep up with. A competent engineer with a good model produces thousands of lines a day; human review is sequential and attention-limited and scales linearly. The gap is widening every quarter.

The standard answer — "make the AI better and eventually trust it" — misreads the situation. Current human code review is mostly theater. LGTM culture, rubber-stamping, blameless postmortems. The accountability we believe is anchoring careful review is largely fictional. We've spent two decades quietly hollowing out the practice of careful per-line review while preserving its appearance.

So the real question isn't "when is AI good enough to replace careful human review." It's "how do we develop a process that's actually verifiable, regardless of whether humans or AI run it." That process should let humans incrementally cede review to machines as confidence accumulates — the same trajectory autonomous vehicles followed, where safety drivers stayed in the seat for years while the system's decisions were compared to what a human would have done, until the human eventually wasn't needed.

`CHANGE_INTENT.md` is one piece of that process. It's the macro-level artifact that captures what a change is supposed to do in a form that both humans and machines can verify against.
