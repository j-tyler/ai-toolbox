...
**Status: applied and verified.** design.md is the restructured version (Part 2's spine and cuts, Part 1's design.md entries); the change-intent README is a directory card; the notes.md trims and voice fixes, the mechanics/README.md boundary reword, and the instrument-preamble, working-in-public, and root-README voice items are all in. A verification sweep of every Part 1 term over the finished corpus found five residuals, since fixed ("will surface it" → "will catch it"; "has surfaced in the conversation" → "has shown in the conversation"; "the signal is actionable" → "the diagnosis is actionable"; "spend their attention" → "focus their attention"; the two instrument "seat" headings aligned to "role," closing the P1 optional item). Confirmed as deliberate keeps: the keep-list terms; "Signals to…" as a verb; "observable surfaces" and "semantic amendment" as terms of art; plain-English uses of "shaped"/"linearly"; and the fork test saying "options" in design.md while the instruments say "branches" (P8, option a). This file remains as the record of the editing program; nothing in it is open.
