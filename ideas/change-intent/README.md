...
Each section serves a distinct downstream purpose: Why carries motivation to future readers; acceptance criteria are mechanically checked test-by-test; invariants invite property thinking across the diff; out of scope signals conscious exclusion to every downstream consumer; amendments preserve the chain of custody, recording how the contract was repaired when implementation proved it wrong. A section being absent means there's nothing for it to hold, not that the author skipped it.
